import React, { useEffect, useState } from "react";
import photo from "../assets/vetkim1.jpg";
import Clinic from "../components/clinic"; 
import { IoSearchOutline } from "react-icons/io5";
import Footer from "../components/footer";

interface IClinic {
  clinic_id: number;
  clinic_name: string;
  province: string;
}

function ClinicsPage() {  
  const [clinics, setClinics] = useState<IClinic[]>([]);
  const [loading, setLoading] = useState<boolean>(true); // Yükleme durumu ekledik

  useEffect(() => {
    const fetchClinics = async () => {
      try {
        const response = await fetch("http://localhost:8000/api/clinics"); 
        if (!response.ok) {
          throw new Error("API'den veri alınamadı!");
        }
        const data = await response.json();
        setClinics(data);
      } catch (error) {
        console.error("Klinik verisi alınırken hata oluştu:", error);
        setClinics([]); 
      } finally {
        setLoading(false); // Yükleme durumu kapatılıyor
      }
    };

    fetchClinics();
  }, []);

  return (
    <div className="px-5 lg:px-20">
      <div className="w-full text-center">
        <img className="w-30 lg:w-60 mx-auto" src={photo} alt="VetKim Logo" />
      </div>

      <div className="mt-3 w-4/5 lg:w-3/5 mx-auto flex flex-row gap-2 px-3 py-1 items-center bg-gray-100 rounded">
        <IoSearchOutline />
        <input className="w-full" type="text" placeholder="Search veterinary clinic" />
      </div>

      <div className="grid grid-cols-1 mt-16 w-full md:grid-cols-2 xl:grid-cols-3 gap-8 mb-8">
        {loading ? (
          <p className="text-center text-gray-500">Klinik verileri yükleniyor...</p>
        ) : clinics.length > 0 ? (
          clinics.map((clinic) => (
            <Clinic clinic={clinic} key={clinic.clinic_id} />
          ))
        ) : (
          <p className="text-center text-gray-500">Henüz klinik bilgisi yüklenmedi.</p>
        )}
      </div>

      <Footer />
    </div>
  );
}

export default ClinicsPage;

